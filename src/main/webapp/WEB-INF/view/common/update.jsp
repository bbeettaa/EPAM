...
<form method="post" action="">
    <input type="hidden" name="action" value="updateUser">
            ...
            <option value="user" ${requestScope.user.role=='USER'?'selected':''}><fmt:message key="label.role.user"/></option>
            <option value="speaker" ${requestScope.user.role=='SPEAKER'?'selected':''}><fmt:message key="label.role.speaker"/></option>
            <c:choose>
                <c:when test="${sessionScope.role == 'ADMIN'}">
                    <option value="moderator" ${requestScope.user.role=='MODERATOR'?'selected':''}><fmt:message key="label.role.moderator"/></option>
                </c:when>
            </c:choose>
        </select><br><br>
            ...
            <input type="hidden" name="roleToSet" value="${requestScope.user.role}"/>
            <input type="number" hidden name="id" value="${requestScope.user.id}"/>
            <input type="submit" value="<fmt:message key="label.button.update"/>" name="Update"><br>
        </form>
    </div>
</body>
</html>